...
/**
 * AI Cafe Website - Components JavaScript
...
const InteractiveComponents = {
  ...
  setupMeetingFilters() {
    const filterButtons = document.querySelectorAll('[data-filter-meetings]');
    ...
    filterButtons.forEach(button => {
      button.addEventListener('click', () => {
        ...
        meetingItems.forEach(item => {
          if (filterValue === 'all' || item.dataset.meetingId === filterValue) {
    ...
    return `
      <article class="team-card" data-searchable="${member.name} ${member.role} ${member.description}">
        ...
        <div class="team-contributions">
          ${member.contributions.map(contribution => `<span class="contribution">${contribution}</span>`).join('')}
        </div>
      </article>
    `;
  },
