...
export class JSONAdapter {
     ...
     */
    async getMeetings() {
        const response = await this.fetchJSON('meetings.json');
     ...
     */
    async getTools() {
        const response = await this.fetchJSON('tools.json');
    ...
    async searchMeetings(query, filters) {
        ...
        if (query.trim()) {
            ...
            results = results.filter(meeting => meeting.title.toLowerCase().includes(normalizedQuery) ||
                meeting.description.toLowerCase().includes(normalizedQuery) ||
                meeting.topics.some(topic => topic.toLowerCase().includes(normalizedQuery)) ||
                meeting.presenters.some(presenter => presenter.name.toLowerCase().includes(normalizedQuery)) ||
                meeting.tools.some(tool => tool.toLowerCase().includes(normalizedQuery)));
        }
        // Apply filters
        if (filters) {
            if (filters.presenter) {
                results = results.filter(meeting => meeting.presenters.some(presenter => presenter.name.toLowerCase().includes(filters.presenter.toLowerCase())));
            }
            if (filters.tool) {
                results = results.filter(meeting => meeting.tools.some(tool => tool.toLowerCase().includes(filters.tool.toLowerCase())));
            }
            if (filters.topic) {
                results = results.filter(meeting => meeting.topics.some(topic => topic.toLowerCase().includes(filters.topic.toLowerCase())));
            }
    ...
    async searchTools(query, filters) {
        ...
        if (query.trim()) {
            const normalizedQuery = query.toLowerCase();
            results = results.filter(tool => tool.name.toLowerCase().includes(normalizedQuery) ||
                tool.description.toLowerCase().includes(normalizedQuery) ||
                tool.vendor.toLowerCase().includes(normalizedQuery) ||
                tool.tags.some(tag => tag.toLowerCase().includes(normalizedQuery)) ||
                tool.features.some(feature => feature.name.toLowerCase().includes(normalizedQuery) ||
        ...
        if (filters) {
            ...
            if (filters.vendor) {
                results = results.filter(tool => tool.vendor.toLowerCase().includes(filters.vendor.toLowerCase()));
            }
...
JSONAdapter.CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
